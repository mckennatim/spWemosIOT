...
void Reqs::clpub(char status[20], char astr[140]){
  if (cclient.connected()){
...
void Reqs::pubState(int hc){
  ...
  if((hc & 2) == 2){
    sprintf(astr, "{\"id\":%d, \"darr\":[%d, %d, %d, %d], \"new\":%d}", 1, sr.temp2.temp, sr.temp2.state, sr.temp2.hilimit, sr.temp2.lolimit, sr.temp2.rec);
    clpub(srstate, astr);
    sr.temp2.rec=0;
  }
  if((hc & 4) == 4){
    sprintf(astr, "{\"id\":%d, \"darr\":[%d], \"new\":%d}", 2, sr.timr1.state, sr.timr1.rec);
    clpub(srstate, astr);
    sr.timr1.rec=0;
  }
  if((hc & 8) == 8){
    sprintf(astr, "{\"id\":%d, \"darr\":[%d], \"new\":%d}", 3, sr.timr2.state, sr.timr2.rec);
    clpub(srstate, astr);
    sr.timr2.rec=0;
  }
  if((hc & 16) == 16){
    sprintf(astr,   "{\"id\":%d, \"darr\":[%d], \"new\":%d}", 4, sr.timr3.state, sr.timr3.rec);
    clpub(srstate, astr);
    sr.timr3.rec=0;
  }
}
